...
class ProfileController extends GetxController {
  ...
  getUserProfile() {
    server.getRequest(endPoint: APIList.profile).then((response) {
      ...
      if (response != null && response.statusCode == 200) {
        final jsonResponse = json.decode(response.body);
        final profileData = ProfileModel.fromJson(jsonResponse);
        ...
        // Assign safely
        profileUser = user ?? ProfileUserData();
        profileMerchant = merchant ?? MerchantProfile();
        profileHub = hub ?? ProfileHub();
  ...
  updateUserProfile(filepath,nidpath,tradepath, type) async {
    loader = true;
    Future.delayed(Duration(milliseconds: 10), () {
      update();
    });
