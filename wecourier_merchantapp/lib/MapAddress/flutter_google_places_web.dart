...
class FlutterGooglePlacesWebState extends State<FlutterGooglePlacesWeb> with SingleTickerProviderStateMixin {
  ...
  selectResult(Address clickedAddress) async {
    final Map<String, dynamic> responseLocation = await http.get(Uri.parse('https://maps.googleapis.com/maps/api/place/details/json?place_id=${clickedAddress.addressPlaceId}&key=${widget.apiKey}'), headers: {'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Credentials': 'true', 'Access-Control-Allow-Methods': 'GET,POST,HEAD'}).then((value) => jsonDecode(value.body));
    ...
    setState(()  {
      ...
      FlutterGooglePlacesWeb.value['lat'] = responseLocation['result']['geometry']['location']['lat'].toString()??'';
      FlutterGooglePlacesWeb.value['long'] = responseLocation['result']['geometry']['location']['lng'].toString()??'';
    });
  }
